*-------------------------------------------------------------------------------
* 05_diversification_sectoral.do: Sectoral (industrial) Shannon diversification, level and growth-based.
...
collapse (sum) share_sq_employment, by(city_id city year)
...
collapse (sum) sum_d_employment=d_employment, by(year city city_id)
...
collapse (sum) share_sq, by(city_id city year)
...
collapse (mean) div_emp_diff= inv_share_sq, by(city_id city)
