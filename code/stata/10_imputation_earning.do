*-------------------------------------------------------------------------------
* Filename:     10_imputation_earning.do
* Purpose:      Watson et al. imputation of fishing earnings where revenue is suppressed. NOTE: this is Fishing_revenue_imputation.do, NOT the imputation_earning.do that the original ReadMe names. Only this variant reproduces the avg_fish_ratio_im in the submitted master file (N=210, mean .3978859, min 0); imputation_earning.do adds `drop if fished==0` and merges with keep(3), giving N=180 and a different usigma equation. Verified against the master file 2026-09-15.
